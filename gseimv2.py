...
try:
    ...
    # In PyQt6 enums live on the class, not on QtCore.Qt directly
...
class MultiPlotPopup(BasePopup):
    """Toggle stacked-subplot mode on/off."""
    def __init__(self, main_win):
...
class MultiPlotWindow(QMainWindow):
    """
    Stacked-subplot version: one subplot per selected Y variable,
    all sharing the same X axis.
    """

    def __init__(self, x, x_label, all_series, settings, parent=None):
        """
        all_series: list of (y_array, LineStyling, axis_side)
                    where axis_side is 'L' or 'R'
        """
        super().__init__(parent)
        ...
        for i, (y, style, _side) in enumerate(all_series):
            ...
            ax.label_outer()   # only bottom subplot shows x tick labels
            axes.append(ax)
...
class MainWindow(QMainWindow):
    """
    The control panel.  Everything lives here:
      - Load a .in project file
      - Pick which .dat output file to look at
      - Pick X column and Y columns (with L/R axis assignment)
      - Buttons to open each settings popup
      - "Plot" button that spawns a new independent plot window
    """
    ...
    def _toggle_enabled(self, label, checked):
        ...
        if label == "Multi-plot" and self.x_col != 0:
            QMessageBox.warning(self, "Multi-Plot", "X axis must be 'time' for multi-plot mode.")
