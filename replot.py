...
# ─────────────────────────────────────────────────────────────────────────────
# SECTION 1 – SMALL TOOLBAR (unchanged from original)
# ─────────────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
# SECTION 2 – STANDALONE MATH (ported unchanged from original)
# ─────────────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
# SECTION 3 – SETTINGS DATACLASSES
# Each one is a plain bag of values for one concern.
# No setX() methods needed – just do  obj.field = value  directly.
# ─────────────────────────────────────────────────────────────────────────────
...
class MultiPlotPanel:
    """Configuration for ONE subplot inside multi-plot mode."""
    x_col:       int  = 0
...
class BasePopup(QMainWindow):
    """
    Every settings popup follows the same pattern:
      - show current settings in the widgets  (load_settings)
      - Apply reads widgets → writes back     (apply)
      - Ok = apply + close
      - Cancel = just close
    This base class provides the three buttons and wires them up,
    so each subclass only needs to implement load_settings() and apply().
    """

    def __init__(self, title, parent=None):
        super().__init__(parent)
        self.setWindowTitle(title)
        # Each subclass will place its own widgets into self.content_widget
        # which lives above the button row.
    ...
    def showEvent(self, event):
        """Auto-called by Qt whenever the window is shown."""
        self.load_settings()
        super().showEvent(event)

    def load_settings(self):
        """Override in subclass: push current settings into the widgets."""
        pass

    def apply(self):
        """Override in subclass: read widgets and write back to the settings object."""
        pass


def _color_button(initial_color: str) -> tuple:
    """
    Make a small button that shows a colored square.
    Returns (button, state_dict) where state_dict['color'] holds the current QColor.
    Clicking the button opens the system color picker.
    """
...
class PlotWindow(QMainWindow):
    ...
    def __init__(self, x, x_label,
        ...
        # Kept around so replot() can fade existing lines and rebuild the
        # legend when a new run's data gets overlaid on top later.
        self.ax = ax
        self.ax2 = ax2
        self._lines = collected_lines
        self._labels = collected_labels
        self._legend_settings = lg

    def replot(self, new_x, new_left_series, new_right_series):
        for ln in self.ax.get_lines() + self.ax2.get_lines():
            ln.set_alpha(0.35)
        ...
        lg = self._legend_settings
        self.ax.legend(self._lines, self._labels,
                      loc=lg.location, frameon=lg.frame,
                      fontsize=lg.fontsize, title=lg.title or None,
                      markerfirst=lg.marker_first,
                      markerscale=lg.marker_scale,
...
class MainWindow(QMainWindow):
    ...
    def _build_ui(self):
        ...
        # Settings buttons  (two columns via a grid).
        # Some have a real on/off flag — those get a checkbox next to the
        # button so you can see (and toggle) the state without opening the
        # popup. Line style / Axes / Legend have no on/off concept, so they
        # stay plain buttons. Avg/RMS has TWO independent flags (avg, rms),
        # so one checkbox can't honestly represent it — left as a plain
        # button; open the popup to see which of the two is on.
    ...
    def _y_changed(self, item):
        """Keep L and R mutually exclusive per row. When Fourier mode is on,
        only one Y variable is allowed at all (Fourier/THD is computed for
        one signal at a time), so checking a new one clears every other row."""
    ...
    def _enforce_fourier_single_y(self):
        """If Fourier just got turned on and more than one Y is currently
        selected, keep only the first and tell the user why."""
    ...
    def _plot(self):
        ...
        # Remember this window so the "Replot" button can refresh it later
    ...
    def _replot(self):
        win = self.last_plot_window
        if win is None:
            QMessageBox.information(self, "Replot", "Plot something first.")
            return
        try:
            new_data = load_dat(win.dat_path)
        except (FileNotFoundError, ValueError) as e:
            QMessageBox.warning(self, "Replot", str(e))
            return

        new_x     = new_data[:, win.x_col]
        new_left  = [(new_data[:, r], self.line_styles[r]) for r in win.y_left_rows]
        new_right = [(new_data[:, r], self.line_styles[r]) for r in win.y_right_rows]
